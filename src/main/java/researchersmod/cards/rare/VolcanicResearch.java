...
import researchersmod.powers.ChaoticResearchPower;
...
public class VolcanicResearch extends BaseCard {
    ...
    public VolcanicResearch() {
        ...
        if(ModConfig.oldVolcanicResearch) this.rawDescription = cardStrings.EXTENDED_DESCRIPTION[0];
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (ModConfig.oldVolcanicResearch) {
            Wiz.applyToSelf(new ChaoticResearchPower(p, magicNumber));
        } else {
